USE BACA2
GO
CREATE TRIGGER Tr1 ON tips  --wskazówka musi pojawiæ siê nie póŸniej ni¿ koniec zadania
INSTEAD OF INSERT
AS
	INSERT INTO tips
...
CREATE TRIGGER Tr3 ON messages   --komunikat musi pojawiæ siê w terminie aktywnoœci przedmiotu
INSTEAD OF INSERT
AS
...
CREATE TRIGGER Tr5 ON reply  --odpowiedŸ musi pojawiæ siê po zadaniu pytania i przed zakoñczeniem zadania 
INSTEAD OF INSERT
AS
	INSERT into reply
	SELECT I.reply_essence, I.reply_date, I.questions_question_id, I.users_user_id FROM inserted I
	WHERE I.reply_date > (SELECT Q.question_date FROM questions Q WHERE Q.question_id = I.questions_question_id)
...
CREATE TRIGGER Tr7 ON participant
AFTER UPDATE
AS
	DECLARE cur CURSOR LOCAL
		FOR SELECT subject_subject_id FROM inserted
	OPEN cur;
	DECLARE @subject INT
	FETCH NEXT FROM cur INTO @subject
	WHILE @@FETCH_STATUS = 0
	BEGIN
		IF NOT EXISTS ( SELECT * FROM participant 
						WHERE subject_subject_id = @subject 
						AND role = 'moderator' 
						AND is_active = 1 )
			BEGIN
				RAISERROR('Nie mo¿na usun¹æ ostatniego modeartora przedmiotu', 16, 1)
				ROLLBACK
				BREAK
			END
		FETCH NEXT FROM cur INTO @subject
...
CREATE TRIGGER Tr7a ON participant
AFTER DELETE
AS
	DECLARE cur2 CURSOR LOCAL 
		FOR SELECT subject_subject_id FROM deleted
	OPEN cur2;
	DECLARE @subject INT
	FETCH NEXT FROM cur2 INTO @subject
	WHILE @@FETCH_STATUS = 0
	BEGIN
		IF NOT EXISTS ( SELECT * FROM participant 
						WHERE subject_subject_id = @subject 
...
CREATE TRIGGER Tr9 ON decrease_function
AFTER INSERT
AS
