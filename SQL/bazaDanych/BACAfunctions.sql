...
AS
RETURN
	SELECT A.subject_id, A.subject_name, A.year, P.role FROM ActiveSubjects AS A JOIN participant AS P
...
CREATE FUNCTION userLogin(@userId INT, @password VARCHAR(20))
RETURNS BIT
AS
BEGIN
...
AS
	RETURN
	SELECT 'q' AS type, question_id AS id, question_date AS date, question_essence AS text, users_user_id, U.login_name
...
AS
RETURN
	SELECT E.* FROM Exercise E WHERE E.subject_subject_id = @subjectID
GO
